...
import { platformApi, type AiProviderInfo } from '../services/platformApi';
...
export default function AiConfigSection({ tenantId }: Props) {
  ...
  const loadStatus = async () => {
    setLoading(true);
    try {
      const { data } = await platformApi.getAiStatus();
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await platformApi.updateTenant(tenantId, { aiConfig: { tasks: assignments } });
  ...
  const handleReset = async () => {
    setSaving(true);
    try {
      await platformApi.updateTenant(tenantId, { aiConfig: {} });
  ...
  const handleSetKey = async () => {
    ...
    setSettingKey(true);
    try {
      await platformApi.setAiKey(keyModalProvider, keyValue.trim());
  ...
  const handleRemoveKey = async (providerName: string) => {
    try {
      await platformApi.removeAiKey(providerName);
